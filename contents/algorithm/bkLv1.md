...
3. parseInt 함수를 사용하여 inputData에 저장된 문자열을 정수로 변환합니다. 배열에서 첫번쨰는 0부터 count됩니다.<br/>📍parseInt와 Number의 차이
   <abbr title="parseInt는 주로 문자열을 정수로 변환하는 데 사용됩니다. parseInt는 문자열의 왼쪽에서 오른쪽으로 문자를 읽으며, 유효한 숫자가 아닌 문자를 만나면 그 위치에서 중지하고 그동안 분석한 숫자를 반환합니다. 숫자로 시작하지 않는 문자열을 변환하려고 하면 NaN을 반환합니다.예를 들어, 123abc은 parseInt를 사용하면 123으로 변환되며, abc123은 NaN으로 변환됩니다. Number는 문자열을 숫자로 변환하는 방법입니다. 문자열이 유효한 숫자로 변환될 수 없다면, Number는 NaN을 반환합니다. 예를 들어, Number(123)는 123을 반환하고, Number(123.456)는 123.456을 반환합니다. 그러나 Number(123abc)는 NaN을 반환합니다.">[보기]</abbr> <br/>
